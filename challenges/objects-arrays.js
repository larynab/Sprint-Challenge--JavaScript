...
function getUniversities(arr) {
  arr = universities;
    for(let i = 0; i < 10; i++) {
    universities.splice(i, 0,`${graduates[i].university}`);
    };
};
...
arr = contactInfo;
    ...
    for(let i = 0; i < 10; i++) {
    contactInfo.splice(i, 0,`${graduates[i].first_name} ${graduates[i].email}`);
    };
};
...
const uni = graduates.filter((currentValue) =>{
  return currentValue.university = "Uni"
});
uni.filter(function(item) {
  return item == 'Uni';
});
console.log(uni);
...
function getAnimalname(arr) {
arr = animalNames;

    for(let i = 0; i < 10; i++) {
    animalNames.splice(i, 0,`${zooAnimals[i].animal_name} ${zooAnimals[i].scientific_name}`);
    };
};
getAnimalname(animalNames);
